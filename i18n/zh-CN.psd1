# =====================================================================
# Windots 消息目录 - 简体中文 (i18n/zh-CN.psd1)
# =====================================================================
@{
    # --- 配置文件 ---
    'config.notfound'                        = '找不到数据文件: {0}'

    # --- 环境检测 ---
    'detect.winget.loading'                  = '正在获取 winget 已安装列表（首次稍慢）...'

    # --- 代理 ---
    'proxy.set'                              = '已为当前会话设置代理：{0}'

    # --- 交互 UI ---
    'ui.empty'                               = '{0} : 无可选项，跳过'
    'ui.select.hint'                         = '  ↑/↓ 移动  空格 切换  A 全选  N 全不选  Enter 确认  Esc 取消'
    'ui.ctrlc.hint'                          = '再次按 Ctrl+C 退出'
    'ui.select.scroll.up'                    = '  ↑ 还有 {0} 项'
    'ui.select.scroll.down'                  = '  ↓ 还有 {0} 项'
    'ui.select.installed'                    = '[已安装]'
    'ui.select.global'                       = '[G]'
    'ui.select.not.installed'                = '[未安装]'
    'ui.select.unsupported'                  = '[暂不支持]'
    'ui.select.selected.count'               = '（已选中{0}项）'
    'ui.select.locked.hidden'                = '（已隐藏 {0} 个锁定项）'
    'ui.single.hint'                         = '  ↑/↓ 移动  Enter 确认'
    'ui.yesno.invalid'                       = '请输入 y 或 n'
    'ui.text.default'                        = '{0} (默认: {1})'
    'ui.packages.col.no'                     = '#'
    'ui.packages.col.name'                   = '名称'
    'ui.packages.col.name.user'              = '用户包名称'
    'ui.packages.col.name.global'            = '全局包名称'
    'ui.packages.col.desc'                   = '说明'
    'ui.packages.col.deps'                   = '依赖'

    # --- Shim ---
    'shim.notfound'                          = '找不到 shim 源文件：{0}，跳过 windots 注册'
    'shim.copied'                            = 'windots.ps1 已复制到 {0}'
    'shim.cmd.copied'                        = 'windots.cmd 已复制到 {0}'
    'shim.linked'                            = 'windots.ps1 已硬链接到 {0}'
    'shim.cmd.linked'                        = 'windots.cmd 已硬链接到 {0}'
    'shim.skip'                              = '已正确硬链接，跳过：{0}'
    'shim.hardlink.fallback'                 = '硬链接失败，已回退为复制：{0} ({1})'
    'shim.path.added'                        = '已将 {0} 加入用户 PATH'
    'shim.path.reopen'                       = '请重开 PowerShell 窗口后 windots 命令才生效'
    'shim.available'                         = 'windots 已在 PATH 中可用'

    # --- Scoop ---
    'scoop.installed'                        = 'scoop 已安装，跳过'
    'scoop.admin.err'                        = 'scoop 不能在管理员窗口中安装。请使用普通用户 PowerShell 窗口重新运行。'
    'scoop.installing'                       = '正在安装 scoop（来源：{0}）...'
    'scoop.mirror.already'                   = 'scoop 镜像已是 gitee，跳过'
    'scoop.mirror.switching'                 = '正在切换 scoop 到 gitee 镜像...'
    'scoop.mirror.repo.pending'              = 'SCOOP_REPO 尚未写入 scoop config，镜像切换可能失败'
    'scoop.mirror.done'                      = 'scoop 镜像已切换'
    'scoop.config.setting'                   = 'scoop config {0} = {1}'
    'scoop.bucket.adding'                    = '正在添加 scoop bucket：{0}'
    'scoop.bucket.installed'                 = 'scoop bucket: {0} 已存在，跳过'
    'scoop.app.installed'                    = 'scoop: {0} 已安装，跳过'
    'scoop.app.installing'                   = '正在通过 scoop 安装：{0}'
    'scoop.app.fail'                         = 'scoop 安装失败：{0} (exit={1})'
    'scoop.app.ok'                           = 'scoop: {0} 安装完成'
    'scoop.app.not.installed'                = 'scoop: {0} 未安装，跳过'
    'scoop.app.uninstalling'                 = '正在通过 scoop 卸载：{0}'
    'scoop.app.uninstall.fail'               = '{0} 卸载失败：{1}'
    'scoop.app.uninstall.still'              = '{0} 卸载未完成：{1}'
    'scoop.app.migrate.fail'                 = '{0} 迁移失败：{1}'
    'scoop.app.migrate.incomplete'           = '{0} 迁移未完成：{1}'
    'scoop.app.uninstalled'                  = 'scoop: {0} 已卸载'
    'scoop.scope.user'                       = '用户'
    'scoop.scope.global'                     = '全局'
    'scoop.scope.migrating'                  = 'scoop: {0} 安装位置迁移 ({1} → {2})'
    'scoop.global.admin.err'                 = '全局安装 {0} 需要管理员权限，请安装 sudo 包或以管理员身份运行 windots'
    'scoop.global.using.sudo'                = '通过 sudo 执行全局操作：{0}'

    # --- 备份 ---
    'backup.dir.zip'                         = '目录已备份为 zip：{0}'
    'backup.file.bak'                        = '文件已备份：{0}'

    # --- 链接 ---
    'links.hardlink.ok'                      = '硬链接：{0} ⇄ {1}'
    'links.hardlink.fail'                    = '硬链接失败（{0}），尝试符号链接...'
    'links.symlink.ok'                       = '符号链接：{0} → {1}'
    'links.symlink.fail.fallback'            = '符号链接失败（{0}），改为复制...'
    'links.symlink.fail'                     = '创建符号链接失败：{0} ({1})'
    'links.copy.ok'                          = '已复制：{0} ← {1}'
    'links.copy.fail'                        = '复制失败：{0} ({1})'
    'links.src.missing'                      = '源不存在，跳过：{0}'
    'links.glob.no.match'                    = 'Dotfiles glob 无匹配文件，跳过：{0}'
    'links.glob.dest.mismatch'               = 'Dotfiles Dest 须含与 Src 对应的 glob 通配符：{0}'
    'links.glob.star.mismatch'               = 'Dotfiles Src/Dest 中 * 数量不一致：{0} → {1}'
    'links.dest.ok'                          = '链接已正确：{0}'
    'links.dest.hardlink.ok'                 = '硬链接已正确：{0}'
    'links.dest.keep'                        = '保留原配置，跳过：{0}'
    'links.dir.no.hardlink'                  = '目录无法硬链接，尝试符号链接：{0}'
    'links.symlink.devmode.title'            = '=== 需要开发者模式或管理员权限才能创建符号链接 ==='
    'links.symlink.devmode.guide1'           = '请前往：设置 → 隐私与安全性 → 开发者专用 → 开启「开发人员模式」'
    'links.symlink.devmode.guide2'           = '开启后按 R 重试检测，或按 C 改为复制文件模式'
    'links.symlink.devmode.ok'               = '已检测到开发者模式，使用符号链接'
    'links.symlink.devmode.retry'            = '未检测到开发者模式，请确认已开启后再按 R'
    'links.symlink.copy.chosen'              = '已选择复制模式，所有配置文件将使用复制方式应用'

    # --- Doctor ---
    'doctor.title'                           = '=== windots doctor ==='
    'doctor.ps.name'                         = 'PowerShell 版本'
    'doctor.ps.warn.suffix'                  = ' (建议升级到 7+)'
    'doctor.scoop.name'                      = 'scoop'
    'doctor.scoop.installed'                 = '已安装'
    'doctor.scoop.missing'                   = '未找到'
    'doctor.git.name'                        = 'git'
    'doctor.git.installed'                   = '已安装'
    'doctor.git.missing'                     = '未找到'
    'doctor.windots.name'                    = 'windots shim'
    'doctor.windots.missing'                 = '未注册，运行 windots init 重新注册'
    'doctor.state.name'                      = 'state 文件'
    'doctor.state.missing'                   = '不存在，首次运行会自动创建'
    'doctor.persist.dual.name'               = 'Persist 双端安装'
    'doctor.persist.dual.detail'             = '{0} 同时安装在 user 与 global：{1} | {2}'
    'doctor.persist.empty.name'              = 'Persist 漂移'
    'doctor.persist.empty.detail'            = '{0} intent 侧 persist 为空，另一侧有数据：{1} ← {2}；建议 link 时同步或 windots migrate'

    # --- 运行总结 ---
    'summary.title'                          = '=== 运行总结 ==='
    'summary.title.uninstall'                = '=== 卸载总结 ==='
    'summary.status.group.ok'                = '--- 安装成功 ---'
    'summary.status.group.ok.uninstall'      = '--- 卸载成功 ---'
    'summary.status.group.skip'              = '--- 已安装，跳过 ---'
    'summary.status.group.skip.uninstall'    = '--- 未安装或被依赖，跳过 ---'
    'summary.status.group.fail'              = '--- 安装失败 ---'
    'summary.status.group.fail.uninstall'    = '--- 卸载失败 ---'
    'summary.status.group.migrate.ok'        = '--- 迁移成功 ---'
    'summary.status.group.migrate.fail'      = '--- 迁移失败 ---'
    'summary.block.pkgmgr'                   = '包管理器'
    'summary.block.packages'                 = 'scoop 包'
    'summary.block.migrate'                  = 'scope 迁移'
    'summary.block.config'                   = '配置文件'
    'summary.block.chezmoi'                  = 'chezmoi'
    'summary.label.mirror'                   = 'gitee 镜像'
    'summary.label.scoop.config'             = 'scoop 配置'
    'summary.label.buckets'                  = 'scoop buckets'
    'summary.label.chezmoi'                  = 'chezmoi init'
    'summary.detail.whatif'                  = '预演模式'
    'summary.detail.scoop.skip'              = '已安装，跳过'
    'summary.detail.scoop.config.none'       = '未配置 SetConfig，跳过'
    'summary.detail.scoop.config.skip'       = 'scoop config 已是最新，跳过'
    'summary.detail.scoop.config.ok'         = '已写入 {0} 项 scoop config'
    'summary.detail.scoop.ok'                = '安装完成'
    'summary.detail.scoop.fail'              = '安装失败'
    'summary.detail.mirror.skip'             = '镜像已是 gitee，跳过'
    'summary.detail.mirror.ok'               = '镜像已切换'
    'summary.detail.bucket.none'             = '未配置 bucket，跳过'
    'summary.detail.bucket.skip'             = '所有 bucket 已存在，跳过'
    'summary.detail.bucket.ok'               = '已添加 {0} 个 bucket'
    'summary.detail.app.skip'                = '已安装，跳过'
    'summary.detail.app.ok'                  = '安装完成'
    'summary.detail.app.fail'                = '安装失败'
    'summary.detail.migrate.fail'            = '迁移失败'
    'summary.detail.app.uninstall.fail'      = '卸载失败'
    'summary.detail.app.uninstall.still'     = '卸载未完成，包仍存在于当前 scope'
    'summary.detail.migrate.incomplete'      = '迁移未完成，包仍存在于当前 scope'
    'summary.detail.app.not.installed'       = '未安装，跳过'
    'summary.detail.app.uninstalled'         = '已卸载'
    'summary.detail.app.uninstall.blocked'   = '仍被 {0} 依赖，跳过'
    'summary.detail.scoop.global.admin'      = '全局安装需要管理员权限'
    'summary.detail.config.missing'          = '源不存在，跳过'
    'summary.detail.config.link.ok'          = '链接已正确'
    'summary.detail.config.hardlink.ok'      = '硬链接已正确'
    'summary.detail.config.keep'             = '保留原配置，跳过'
    'summary.detail.config.hardlink'         = '硬链接已应用'
    'summary.detail.config.symlink'          = '符号链接已应用'
    'summary.detail.config.copy'             = '已复制'
    'summary.detail.config.fail'             = '应用失败'
    'summary.detail.chezmoi.skip'            = '未找到 chezmoi，跳过'
    'summary.detail.chezmoi.ok'              = '初始化完成'
    'summary.detail.chezmoi.fail'            = '初始化失败'
    'summary.detail.raw.unknown'             = 'exit={0}（无详细错误输出）'
    'summary.done'                           = '完成: {0} 成功  {1} 跳过  {2} 失败'
    'summary.done.with_migrate'              = '完成: {0} 成功  {1} 跳过  {2} 失败  |  迁移: {3} 成功  {4} 失败'
    'summary.done.uninstall'                 = '完成: {0} 卸载  {1} 跳过  {2} 失败'
    'summary.log'                            = '详细日志：{0}'
    'summary.hint'                           = '后续请在新终端使用 windots 命令管理环境（init / install / uninstall / update / migrate / clean / link / harvest / doctor / cd）'

    # --- 交互配置 ---
    'interactive.title'                      = '=== Windots 交互配置 ==='
    'interactive.cancelled'                  = '已取消操作。'
    'interactive.repo'                       = '仓库目录：{0}'
    'interactive.log'                        = '日志文件：{0}'
    'interactive.step.proxy'                 = '--- 1/6 代理配置 ---'
    'interactive.proxy.desc'                 = '国内访问 GitHub 可能较慢，可配置代理加速安装。'
    'interactive.proxy.prompt'               = '是否使用代理？'
    'interactive.proxy.url.prompt'           = '代理地址 (默认 socks5://127.0.0.1:10808)'
    'interactive.step.pkgmgr'                = '--- 2/6 包管理器 ---'
    'interactive.step.scoop.paths'           = '--- Scoop 安装路径 ---'
    'interactive.scoop.paths.missing'        = 'settings.psd1 中 Scoop.SetConfig 未配置 root_path 或 global_path：{0}'
    'interactive.scoop.paths.hint'           = '请编辑上述文件，在 Scoop.SetConfig 中设置 ROOT_PATH 与 GLOBAL_PATH 后返回继续。'
    'interactive.scoop.paths.ready.prompt'   = '路径已配置完成，继续初始化？'
    'interactive.scoop.paths.review'         = '当前 settings.psd1 中的 Scoop 安装路径：'
    'interactive.scoop.paths.confirm.prompt' = '确认使用以上路径继续？'
    'interactive.pkgmgr.title'               = '选择包管理器：'
    'interactive.step.mirror'                = '--- 3/6 Scoop 镜像 ---'
    'interactive.mirror.desc'                = '切换到 gitee 镜像可加快 scoop 本体和 bucket 的下载速度。'
    'interactive.mirror.prompt'              = '是否切换 scoop 到 gitee 国内镜像？'
    'interactive.step.packages'              = '--- 4/6 选择安装包 ---'
    'interactive.packages.title'             = '选择安装包：'
    'ui.select.tree.hint.move'               = '  J/K(↓/↑) 移动  D/U(→/←) 跳组  Shift+D/U 底/顶组  F 折叠/展开  Shift+F 折叠/展开全部  H 隐藏/显示'
    'ui.select.tree.hint.select'             = '  空格 切换  A/N 当前组  Shift+A/N 全部  G 切换全局安装  Enter 确认  Esc 取消'
    'ui.select.tree.hint.select.noglobal'    = '  空格 切换  A/N 当前组  Shift+A/N 全部  Enter 确认  Esc 取消'
    'interactive.step.chezmoi'               = '--- 5/6 chezmoi 配置同步 ---'
    'interactive.chezmoi.prompt'             = '是否使用 chezmoi 同步跨平台配置（nvim/starship/wezterm 等）？'
    'interactive.chezmoi.user.prompt'        = 'GitHub 用户名或仓库地址'
    'interactive.chezmoi.apply.prompt'       = '是否立即 --apply（拉取并应用配置）？'
    'interactive.step.config'                = '--- 6/6 配置文件应用方式 ---'
    'interactive.conflict.title'             = '已存在的配置文件如何处理？'
    'interactive.conflict.overwrite'         = '直接覆盖（默认）'
    'interactive.conflict.backup'            = '备份后覆盖'
    'interactive.conflict.keep'              = '保留原文件'
    'interactive.linkmode.title'             = '如何应用配置文件？'
    'interactive.linkmode.hardlink'          = '硬链接（推荐，需源与目标同盘）'
    'interactive.linkmode.symlink'           = '软链接'
    'interactive.linkmode.copy'              = '复制文件'
    'interactive.plan.title'                 = '=== 执行计划摘要 ==='
    'interactive.plan.proxy'                 = '代理      : {0}'
    'interactive.plan.mirror'                = 'scoop 镜像: {0}'
    'interactive.plan.packages'              = '安装包    : {0} 项 (scoop {1} 个)'
    'interactive.plan.packages.user'         = '用户安装包: {0} 项 (scoop {1} 个)'
    'interactive.plan.packages.global'       = '全局安装包: {0} 项 (scoop {1} 个)'
    'interactive.plan.scoop.root'            = 'root_path   : {0}'
    'interactive.plan.scoop.global_path'     = 'global_path : {0}'
    'interactive.plan.scoop.path.unset'      = '(未配置)'
    'interactive.plan.scoop.repo'            = 'scoop 镜像  : {0}'
    'interactive.plan.scoop.repo.unset'      = '(未配置)'
    'interactive.plan.scoop.repo.official'   = '官方源'
    'interactive.plan.scoop.path.default'    = '{0} (默认)'
    'interactive.plan.chezmoi'               = 'chezmoi   : {0}'
    'interactive.plan.conflict'              = '配置冲突  : {0}'
    'interactive.plan.linkmode'              = '配置模式  : {0}'
    'interactive.state.saved'                = '配置已保存：{0}'
    'interactive.plan.proxy.none'            = '(无)'
    'interactive.plan.chezmoi.skip'          = '(跳过)'
    'interactive.plan.mirror.enabled'        = '启用 (gitee)'
    'interactive.plan.mirror.disabled'       = '禁用'

    # --- 初始化流程 ---
    'init.step.scoop'                        = '--- 安装 scoop ---'
    'init.step.scoop.config'                 = '--- 写入 scoop 配置 ---'
    'init.scoop.admin.err'                   = 'scoop 必须在普通用户窗口安装，当前为管理员窗口，跳过。'
    'init.step.mirror'                       = '--- 切换 scoop 镜像 ---'
    'init.step.buckets'                      = '--- 安装 scoop buckets ---'
    'init.step.packages'                     = '--- 安装 scoop 包 ---'
    'init.step.config'                       = '--- 应用配置文件 ---'
    'init.config.src.skip'                   = '配置源不存在，跳过：{0}'
    'init.step.chezmoi'                      = '--- 初始化 chezmoi ---'
    'init.chezmoi.missing'                   = 'chezmoi 未找到，跳过（请确认已在包列表中选择 chezmoi）'
    'init.chezmoi.init'                      = 'chezmoi init {0}'

    # --- 包安装流程 ---
    'install.interactive.title'              = '=== Windots 包选择 ==='
    'install.step.packages'                  = '--- 安装 scoop 包 ---'
    'install.step.config'                    = '--- 应用配置文件 ---'
    'install.config.src.skip'                = '配置源不存在，跳过：{0}'
    'install.plan.title'                     = '=== 安装计划摘要 ==='
    'install.plan.packages'                  = '安装包    : {0} 项 (scoop {1} 个)'
    'install.plan.packages.user'             = '用户安装包: {0} 项 (scoop {1} 个)'
    'install.plan.packages.global'           = '全局安装包: {0} 项 (scoop {1} 个)'
    'install.plan.added'                     = '新增包    : {0}'
    'install.plan.added.none'                = '新增包    : (无)'
    'install.state.missing.err'              = '未找到 state 文件，请先运行 windots init 完成初始化'

    # --- 包卸载流程 ---
    'uninstall.interactive.title'            = '=== Windots 包卸载 ==='
    'uninstall.interactive.step.packages'    = '--- 选择待卸载包（勾选即卸载）---'
    'uninstall.plan.title'                   = '=== 卸载计划摘要 ==='
    'uninstall.plan.remaining'               = '保留包    : {0} 项 (scoop {1} 个)'
    'uninstall.plan.remaining.user'          = '保留用户包: {0} 项 (scoop {1} 个)'
    'uninstall.plan.remaining.global'        = '保留全局包: {0} 项 (scoop {1} 个)'
    'uninstall.plan.removed'                 = '移除包    : {0}'
    'uninstall.plan.removed.none'            = '移除包    : (无)'
    'uninstall.plan.apps'                    = '卸载 scoop: {0}'
    'uninstall.plan.apps.none'               = '卸载 scoop: (无)'
    'uninstall.blocked.app'                  = '无法卸载 {0}：仍被 {1} 依赖'
    'uninstall.step.packages'                = '--- 卸载 scoop 包 ---'
    'uninstall.none.selected'                = 'state 中无已选包，无需卸载'
    'uninstall.nothing.todo'                 = '未选择移除任何包，跳过卸载'
    'uninstall.state.missing.err'            = '未找到 state 文件，请先运行 windots init 完成初始化'

    # --- 包 scope 迁移 ---
    'migrate.interactive.title'              = '=== Windots 包安装位置迁移 ==='
    'migrate.interactive.step.packages'      = '--- 选择待迁移包（G 切换目标 scope）---'
    'migrate.packages.title'                 = '选择迁移包：'
    'migrate.plan.title'                     = '=== 迁移计划摘要 ==='
    'migrate.plan.item'                      = '  {0} : {1} → {2}'
    'migrate.plan.confirm.prompt'            = '确认执行以上迁移？'
    'migrate.prompt.forgot_g.hint'           = '以下包可能未切换 G 目标 scope，建议迁移：'
    'migrate.prompt.forgot_g.prompt'         = '是否将以上包迁移至另一 scope？'
    'migrate.step.packages'                  = '--- 迁移 scoop 包 ---'
    'migrate.none.selected'                  = 'state 中无已选包，无需迁移'
    'migrate.nothing.todo'                   = '未选择迁移项或所选包已在目标 scope，跳过'
    'migrate.scoop.missing'                  = 'scoop 未安装，无法迁移'
    'migrate.state.missing.err'              = '未找到 state 文件，请先运行 windots init 完成初始化'

    # --- 清理 ---
    'clean.title'                            = '=== windots clean ==='
    'clean.logs.done'                        = '已清理 logs 目录 ({0} 项)'
    'clean.logs.empty'                       = 'logs 目录为空，跳过'
    'clean.backup.done'                      = '已清理 backup 目录 ({0} 项)'
    'clean.backup.pruned'                    = '已删除 {0} 份旧备份，保留最近一份：{1}'
    'clean.backup.kept'                      = 'backup 仅有一份，已保留：{0}'
    'clean.backup.empty'                     = 'backup 目录为空，跳过'
    'clean.state.done'                       = '已删除 state：{0}'
    'clean.state.missing'                    = 'state 文件不存在，跳过'
    'clean.state.hint'                       = '提示：使用 windots clean -All 可同时删除 state'

    # --- 更新 ---
    'update.title'                           = '=== windots update ==='
    'update.windots.step'                    = '--- 更新 windots ---'
    'update.windots.convert.prompt'          = '当前安装不是 git 仓库，是否迁移为 git 仓库并同步最新版本？'
    'update.windots.convert.skip'            = '已跳过 windots 迁移'
    'update.windots.convert.running'         = '正在克隆 {0} ...'
    'update.windots.convert.ok'              = '已迁移为 git 仓库并同步完成'
    'update.windots.convert.fail'            = 'git clone 失败 (exit={0})'
    'update.windots.convert.backingup'       = '正在备份旧目录到 {0} ...'
    'update.windots.convert.bak'             = '旧目录已备份：{0}'
    'update.windots.repo.missing'            = 'settings.psd1 未配置 Repository.Url，跳过 windots 自更新'
    'update.windots.git.missing'             = '未找到 git，跳过 windots 自更新'
    'update.windots.running'                 = '正在 git pull 更新 windots...'
    'update.windots.ok'                      = 'windots 已更新'
    'update.windots.fail'                    = 'git pull 失败 (exit={0})'
    'update.scoop.missing'                   = 'scoop 未安装，跳过包更新'
    'update.scoop.running'                   = '更新所有 scoop 包...'
    'update.chezmoi.running'                 = '更新 chezmoi 配置...'
    'update.done'                            = 'update 完成'

    # --- 链接重建 ---
    'link.title'                             = '=== windots link ==='
    'link.src.skip'                          = '源不存在，跳过：{0}'
    'link.persist.sync.prompt'               = '检测到 {0} 的 intent 侧 persist 为空，另一侧有数据。是否将 {1} 同步到 {2}？'
    'link.persist.sync.plan'                 = '[WhatIf] 将同步 {0} persist：{1} → {2}'
    'link.persist.sync.done'                 = '已同步 {0} persist：{1} → {2}'
    'link.persist.sync.skipped'              = '已跳过 {0} persist 同步，按 intent scope 继续链接'

    # --- 反向回收 ---
    'harvest.title'                          = '=== windots harvest ==='
    'harvest.confirm'                        = '将回收 {0} 个运行态配置到 dotfiles 入库文件（将覆盖未提交改动）。继续？'
    'harvest.nothing'                        = '无需回收：所有配置已是最新或仍正确链接'
    'harvest.skip.linked'                    = '仍正确链接，跳过：{0}'
    'harvest.skip.nodest'                    = '不存在，跳过：{0}'
    'harvest.skip.unchanged'                 = '内容相同，跳过：{0}'
    'harvest.skip.dir'                       = 'Src 是目录，跳过：{0}'
    'harvest.done'                           = '已回收：{0} → {1}'
    'harvest.plan'                           = '[WhatIf] 将回收：{0} → {1}'

    # --- 主流程 ---
    'setup.ps7.required'                     = "Windots setup 需要 PowerShell 7+（pwsh）。`n请先运行 install.ps1，或使用：pwsh -File setup.ps1"
    'setup.state.title'                      = '=== 发现已保存的配置 ==='
    'setup.state.timestamp'                  = '上次保存时间 : {0}'
    'setup.state.proxy'                      = '代理         : {0}'
    'setup.state.mirror'                     = 'scoop 镜像   : {0}'
    'setup.state.packages'                   = '安装包       : {0} 项 (scoop {1} 个)'
    'setup.state.packages.user'              = '用户安装包   : {0} 项 (scoop {1} 个)'
    'setup.state.packages.global'            = '全局安装包   : {0} 项 (scoop {1} 个)'
    'setup.state.chezmoi'                    = 'chezmoi      : {0}'
    'setup.state.conflict'                   = '配置冲突     : {0}'
    'setup.state.linkmode'                   = '配置模式     : {0}'
    'setup.state.use.prompt'                 = '是否使用以上配置直接开始初始化？'
    'setup.state.missing.err'                = '未找到 state 文件，请先运行 windots init 完成初始化'
    'setup.reconfigure.init.hint'            = '-Reconfigure 仅对 init 有效，请使用 windots init -Reconfigure'
    'setup.cmd.unknown'                      = '未知子命令：{0}'
    'setup.cmd.hint'                         = '可用子命令：init, install (i), uninstall (rm), update (up), migrate, clean, link, harvest (hv), doctor, cd'

    # --- cd ---
    'cd.done'                                = '已进入 {0}'

    # --- 包分组 ---
    'pkg.group.apps'                         = '桌面软件'
    'pkg.group.apps.dev'                     = '开发工具'
    'pkg.group.apps.editor'                  = '编辑器与笔记'
    'pkg.group.apps.productivity'            = '效率增强'
    'pkg.group.apps.remote'                  = '远程与网络'
    'pkg.group.apps.utils'                   = '实用工具'
    'pkg.group.apps.wm'                      = '窗口与状态栏'
    'pkg.group.cli'                          = 'CLI 命令行'
    'pkg.group.cli.base'                     = '基础工具'
    'pkg.group.cli.plus'                     = '增强替代'
    'pkg.group.dev'                          = '开发与配置'
    'pkg.group.dev.runtime'                  = '语言运行时'
    'pkg.group.dev.tools'                    = '开发工具'
    'pkg.group.fonts'                        = '字体'
    'pkg.group.session'                      = '终端与会话'
    'pkg.group.session.enhance'              = '体验增强'
    'pkg.group.session.term'                 = '终端与 Shell'

    # --- 包描述 ---
    'pkg.7zip.desc'                          = '压缩/解压工具'
    'pkg.adb.desc'                           = 'Android SDK 平台工具（adb/fastboot）'
    'pkg.alacritty.desc'                     = 'GPU 加速终端模拟器'
    'pkg.altsnap.desc'                       = '窗口布局辅助（Alt+拖拽）'
    'pkg.apifox.desc'                        = 'API 文档、调试与 Mock 平台'
    'pkg.aria2.desc'                         = '多线程下载工具'
    'pkg.atuin.desc'                         = 'Shell 历史同步与搜索'
    'pkg.bat.desc'                           = '带语法高亮的 cat 替代'
    'pkg.bottom.desc'                        = '系统资源监控 TUI'
    'pkg.btop.desc'                          = '系统资源监控 TUI'
    'pkg.cc-switch.desc'                     = 'Claude Code / Codex / Gemini CLI 等供应商切换助手'
    'pkg.curl.desc'                          = '命令行 HTTP 客户端'
    'pkg.dbeaver.desc'                       = '通用数据库管理工具'
    'pkg.delta.desc'                         = 'git diff 高亮增强'
    'pkg.direnv.desc'                        = '目录级环境变量自动加载'
    'pkg.dust.desc'                          = '可视化磁盘占用分析（du 替代）'
    'pkg.duf.desc'                           = '彩色磁盘空间概览（df 替代）'
    'pkg.everything.desc'                    = 'Windows 毫秒级文件名搜索'
    'pkg.everythingtoolbar.desc'             = 'Everything 任务栏搜索集成'
    'pkg.eza.desc'                           = '更好的 ls 替代'
    'pkg.fastfetch.desc'                     = '系统信息快速展示'
    'pkg.fd.desc'                            = '更快的 find 替代'
    'pkg.flow-launcher.desc'                 = '应用启动器'
    'pkg.fnm.desc'                           = '快速 Node.js 版本管理器'
    'pkg.fzf.desc'                           = '模糊搜索工具'
    'pkg.glazewm.desc'                       = 'Windows 平铺式窗口管理器'
    'pkg.glow.desc'                          = '终端 Markdown 渲染与浏览'
    'pkg.helm.desc'                          = 'Kubernetes 包管理器'
    'pkg.hyperfine.desc'                     = '命令行基准测试工具'
    'pkg.InputTip.desc'                      = '输入法状态提示'
    'pkg.jd.desc'                            = 'JSON diff 工具'
    'pkg.JetBrainsMono-NF.desc'              = 'JetBrains Mono Nerd Font'
    'pkg.jq.desc'                            = 'JSON 命令行处理器'
    'pkg.just.desc'                          = '项目命令脚本运行器（justfile）'
    'pkg.komorebi.desc'                      = 'Windows 平铺式窗口管理器'
    'pkg.lazygit.desc'                       = 'Git TUI 客户端'
    'pkg.lsd.desc'                           = '更好的 ls 替代（eza 替代）'
    'pkg.mise.desc'                          = '多语言版本管理器（asdf 替代）'
    'pkg.neovim.desc'                        = '基于 Vim 的现代编辑器'
    'pkg.nushell.desc'                       = '现代结构化 Shell'
    'pkg.obsidian.desc'                      = '本地 Markdown 知识库'
    'pkg.pixpin.desc'                        = '截图 / 贴图 / 录屏工具'
    'pkg.pnpm.desc'                          = '高效的 Node.js 包管理器'
    'pkg.powertoys.desc'                     = 'Microsoft 官方效率工具集'
    'pkg.PSCompletions.desc'                 = 'PowerShell 补全增强'
    'pkg.rider.desc'                         = 'JetBrains .NET IDE'
    'pkg.rio.desc'                           = '现代终端模拟器'
    'pkg.ripgrep.desc'                       = '更快的 grep 替代'
    'pkg.rufus.desc'                         = '可启动 USB 制作工具'
    'pkg.rust.desc'                          = 'Rust 工具链'
    'pkg.rustdesk.desc'                      = '开源远程桌面'
    'pkg.scoop-search.desc'                  = 'Scoop 包模糊搜索'
    'pkg.starship.desc'                      = '跨 shell 提示符'
    'pkg.steampp.desc'                       = 'Steam 工具箱（Watt Toolkit）'
    'pkg.sudo.desc'                          = '以管理员身份运行命令'
    'pkg.sunshine.desc'                      = 'Moonlight 串流 Host'
    'pkg.tacky-borders.desc'                 = '窗口边框美化'
    'pkg.tealdeer.desc'                      = '简化版命令手册（tldr Rust 实现）'
    'pkg.tldr.desc'                          = '简化版命令手册'
    'pkg.trae.desc'                          = 'AI 编程 IDE'
    'pkg.trilium.desc'                       = '分层笔记知识库'
    'pkg.uutils-coreutils.desc'              = 'GNU coreutils 的 Rust 实现'
    'pkg.unigetui.desc'                      = 'Scoop/WinGet 等包管理 GUI'
    'pkg.v2rayn.desc'                        = 'V2Ray/Xray 代理 GUI 客户端'
    'pkg.vim.desc'                           = '经典终端编辑器'
    'pkg.vscode.desc'                        = 'Visual Studio Code 编辑器'
    'pkg.windirstat.desc'                    = '磁盘占用可视化工具'
    'pkg.wsl-dashboard.desc'                 = 'WSL 实例管理面板'
    'pkg.wsl-ui.desc'                        = 'WSL 发行版管理 GUI'
    'pkg.yasb.desc'                          = 'Windows 状态栏'
    'pkg.yazi.desc'                          = '终端文件管理器'
    'pkg.zed.desc'                           = '高性能多人协作代码编辑器'
    'pkg.zebar.desc'                         = 'Windows 状态栏'
    'pkg.zellij.desc'                        = '终端多路复用器'
    'pkg.zoxide.desc'                        = '智能目录跳转（cd 增强）'
}
